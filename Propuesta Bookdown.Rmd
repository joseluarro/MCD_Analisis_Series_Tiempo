...
datos_precios_historicos = pd.read_excel("historico_precios_maracuya.xlsx")

print(datos_precios_historicos.head())
```

```{python}
print(datos_precios_historicos.info())
```

```{python}
print(datos_precios_historicos.shape)
```

```{python}
print(datos_precios_historicos["precio_prom_kg"].describe())
```

```{python}
datos_precios_historicos.plot(x="anio_mes", y="precio_prom_kg")
plt.show()
```
